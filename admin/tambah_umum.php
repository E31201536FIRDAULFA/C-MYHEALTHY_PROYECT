  ...
  <main class="flex-shrink-0">
        ...
        <div class="col-lg-6 mb-4">
          <div class="card border-0 shadow-sm">
            <div class="card-body p-5">
              ...
              <a href="nomor-antrian-umum" class="btn btn-success2 rounded-pill px-4 py-2">
    ...
    <?php
      ...
      $tanggal = $_POST['tanggal'];
			...
			$cek_nik = mysqli_query($conn, "SELECT * FROM poli_umu WHERE NIK='$nik'");
			$row	  = mysqli_num_rows($cek_nik);
						
			if($row > 0){
				echo "<script>alert('NIK tersebut sudah digunakan');</script>";
			}else {
				$query_simpan = mysqli_query($conn, "INSERT INTO poli_umu(NIK, TANGGAL, NAMA, NAMA_POLI) VALUES('$nik', '$tanggal', '$nama', '$namapoli')");
				if(!$query_simpan){
					echo "<script>alert('Data gagal disimpan');document.location.href='tambah_umum.php';</script>";
				}else { 
					echo "<script>alert('Data berhasil disimpan');document.location.href='tambah_umum.php';</script>";
				}
			}
		}
				?>
